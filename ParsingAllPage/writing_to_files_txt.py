...
def main():
    ...
    for i in range(1, num+1):
        #   Sleep: заморозка процесса на указанное количество секунд,
        #   таким образом, каждый запрос к серверу будет разделён определённым количеством времени,
        #   такой метод долгий, но не нуждается в PROXY.
        sleep(10)
        url = f'{URL}?p={i}'
        html = f'{get_url(url)}'
